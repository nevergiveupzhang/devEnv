...
define(function(require, exports, module) {
    ...
    exports.updatePlan = function(callback) {
        ...
        var url = window.hostUrl + 'plan/update';
    ...
    exports.addPlan = function() {
        ...
        var url = window.hostUrl + 'plan/save';
        $.post(url, effectPlan, function(res) {
            if (res) {
                loadUIAndRender('plan','WEB-INF/views/planset/plan.html',function(){
                    Tiger.ep.emit("afterPlanAddSuccess");
                });
                Tiger.ep.all("afterPlanAddSuccess","planPageReady",function() {
        ...
        $.ajax({
            ...
            .done(function(result) {
                if (!result.length) {
                    ...
                    Tiger.bcglObjectId = result[0].id;
    ...
    function fistTimeSaveBcOrder(orderStr) {
        ...
        var objJson = JSON.stringify(new Array(obj));
        $.ajax({
                url: window.hostUrl + 'plan/savePlanOrder',
                type: 'POST',
                async: true,
                data: {
                    "inserted": objJson
    ...
    exports.resetPlan = function() {
        var url = window.hostUrl + "plan/delPlanOrder";
