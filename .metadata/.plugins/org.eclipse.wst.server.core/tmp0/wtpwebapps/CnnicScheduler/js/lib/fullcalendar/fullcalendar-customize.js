/**
 * popupLayer.js弹窗层 modified by giscafer.com
 * //示例6，使用遮罩（此弹窗接口修改完全和排班管理模块绑定，建议其他外部功能不要使用） new
 * PopupLayer({trigger:"#ele6",popupBlk:"#blk6",closeBtn:"#close6",useOverlay:true});
 */
...
var PopupLayer = new Class({
	...
	_init:function(options){
		...
		this.trigger.bind(this.options.eventType,function(e){            // 给触发元素绑定触发事件
			...
			// 清空设置的颜色
			// $(".fc-border-separate").find('.fc-day-content >
		...
		Tiger.paibanClose=this.paibanClose;// 销毁事件
	},
...
/**
 * Lunar.js modified by feifei新增农历及节气等功能
 */
 (function(){
	var DB = {
...
(function(){
	/**
	 * 节日的分类： t:中国传统节日 i:世界性节日 h:中国节假日 c:基督教节日 a:北美节日
	 */
...
/*
 * ! FullCalendar v1.6.4 Docs & License: http://arshaw.com/fullcalendar/ (c)
 * 2013 Adam Shaw
 */
/*
...
function Calendar(element, options, eventSources) {
	...
	function initialRender() {
		...
		content = $("<div class='fc-content' style='position:relative;margin-top:63px;'/>")
			.prependTo(element);
...
function Header(calendar, options) {
	...
	function render() {
		...
		if (sections) {
			element = $("<table class='fc-header' style='width: calc(95% - 63px);position:fixed;z-index:9999;background-color:#FFF;'/>")
				.append(
					$("<tr/>")
...
var dayIDs = ['sun','mon', 'tue', 'wed', 'thu', 'fri','sat','sun'],
	DAY_MS = 86400000,
	HOUR_MS = 3600000,sat
	MINUTE_MS = 60000;
...
function MonthView(element, calendar) {
	...
	function render(date, delta) {
		...
		// 记录下本月的起止时间
		try{
			Tiger.monthStart=start;
			Tiger.monthVisStart=visStart;
			Tiger.monthEnd=new Date(end.getTime()-24*3600000);
			Tiger.monthVisEnd=visEnd;
		}catch(e){
		...
		// 添加viewType，使得日期
		var dayCount=Tiger.monthEnd.getDate();
...
function BasicWeekView(element, calendar) {
	...
	function render(date, delta) {
		...
		// 记录下本周的起止时间
		try{
			Tiger.calenderVisStart=visStart;
			Tiger.calenderVisEnd=visEnd;
		}catch(e){
...
function BasicView(element, calendar, viewName) {
	...
	function buildTable(viewType) {
		...
		Tiger.ep.fire("afterWeekTableRender",null);
	}
...
// 月表头
	function buildMonthHeadHTML() {
		...
		var col;
		var date;
		var monStart=formatDate(Tiger.monthStart, 'yyyy-MM');
		...
		if(currentMon>monStart){
			html +="<th class='fc-sun fc-widget-header' style='color: #25992E;'>评价</th>";
			// "<th class='fc-sun fc-widget-header' tyle='color:
			// #25992E;'>补扣工资(元)</th>"
		}
...
// 月表body Month
	function buildMonthBodyHTML() {
		...
		html += "<tbody>";
		var checkMonth=formatDate(Tiger.monthStart, 'yyyy-MM');
		var currentMon=formatDate(new Date(), 'yyyy-MM');
		Tiger.getPersonCheckInfo(checkMonth);
		// Tiger.getKHInfoStatitics(checkMonth);
		// 获取排班人员
		var checkXhyCache=Tiger.checkPersonCache;
		// 考勤记录
		// Tiger.checkInfoCache=Forestar.App.CheckWidget.instance.checkInfoCache;
		// 排班人员考核得分和补扣工资
		Tiger.checkScoreAndPay={};
		...
		for (var i = 0; i < checkXhyCache.length; i++) {
			...
			if(currentMon>checkMonth){
				// 考核得分和补扣工资计算
				var xhyScoreAndPay=Tiger.checkScoreAndPay[pid] || {};
				...
				// html += "<td class='fc-wed fc-widget-content fc-past'
				// id='check_bkgz_"+pid+"' style='width:35px;height:30px;'>"+
				// '<div><div class="check-pay fc-day-content"
				// evaluate="'+score+'" name="'+pid+'"
				// date-data="'+checkMonth+'" style="height:30px">'+
				// '<div style="position: relative; padding-top: 5px;
				// text-align: center; color: black;"
				// name="cpaycontainer">'+cpay+'</div></div></div></td>';
			}
	...
	function buildMonthCellHTML(date,pid,personName) {
		...
		if(checkDay<=todayDay){
			var checkInfo=Tiger.checkInfoCache[pid];   // 获取改排班人员的当月签到记录;
	...
	function buildWeekHeadHTML(){
		Tiger.getSecondline(Tiger.calenderVisStart,Tiger.calenderVisEnd);
		Tiger.getTxl4Seconline();
		var dayS = $.fullCalendar.formatDate(Tiger.calenderVisStart, "yyyy-MM-dd");
		var groupLeader="未指定",network="未指定",dba="未指定",cn="未指定",sdns="未指定",sos2="未指定",ebero="未指定";
		if(null!=Tiger.tableHeader){
			groupLeader=Tiger.tableHeader.groupLeader;
			groupLeader=buildHeadTitle(groupLeader);
			network=Tiger.tableHeader.network;
			network=buildHeadTitle(network);
			dba=Tiger.tableHeader.dba;
			dba=buildHeadTitle(dba);
			cn=Tiger.tableHeader.cn;
			cn=buildHeadTitle(cn);
			sdns=Tiger.tableHeader.sdns;
			sdns=buildHeadTitle(sdns);
			sos2=Tiger.tableHeader.sos2;
			sos2=buildHeadTitle(sos2);
			ebero=Tiger.tableHeader.ebero;
			ebero=buildHeadTitle(ebero);
		}
		
		var html = "<tr>";
		html +="<th class='fc-sun fc-widget-header fox-top-border fox-v-middle'>二线支持&带班</th>";
		html+="<th class='fc-sun fc-widget-header fox-top-border'>DNS(CN&NEWG)<br/>-<br/>"+cn+"</th>";
		html+="<th class='fc-sun fc-widget-header fox-top-border'>云解析<br/>-<br/>"+sdns+"</th>";
		html += "<th class='fc-sun fc-widget-header fox-top-border'>网络<br/>-<br/>"+network+"</th>";
		html += "<th class='fc-sun fc-widget-header fox-top-border'>DBA<br/>-<br/>"+dba+"</th>";
		html+="<th class='fc-sun fc-widget-header fox-top-border'>监控<br/>-<br/>"+sos2+"</th>";
		html+="<th class='fc-sun fc-widget-header fox-top-border'>EBERO<br/>-<br/>"+ebero+"</th>";
		html += "<th class='fc-sun fc-widget-header fox-top-border'>带班组长<br/>-<br/>"+groupLeader+"</th>";
		html+="</tr>";
		return html;
	}
	
	function buildHeadTitle(nameStr){
		var result="";
		...
		for(var i=0;i<nameArr.length;i++){
			...
			result+=name;
			for(var nameKey in Tiger.txl){
				if(name==nameKey){
					var cellphone=Tiger.txl[nameKey].split(",");
	...
	function buildWeekBodyHTML() {
		...
		html += "<tbody>";
		// 获取排班人员分组
		var xhyGroup=Tiger.pbglXhyCache;
		// 获取班次颜色
		Tiger.pbglBcColorObj=Tiger.pbglBcColorObj;
		Tiger.getScheduleEvents(Tiger.calenderVisStart,Tiger.calenderVisEnd);
		Tiger.xhyEventsObj=Tiger.pbglEventsCache;
		// 行数=人员个数+组数
		rowCnt=Tiger.pbglRowCnt+xhyGroup.length;
		for (var i = 0; i < xhyGroup.length; i++) {
			...
			if(i==0){
				html += "<tr class='fc-week'><td class='fox-top-border fc-wed fc-widget-content fc-past' colspan='8'"+
				" align='center' style='font-size:15px;font-weight:bold'><a href='#' onclick=loadUI('WEB-INF/views/schedule/schedulerWeekGroup.html',"+JSON.stringify(options)+")>"+groupName+"</a></td></tr>";
			}else{
				html += "<tr class='fc-week'><td class='fc-wed fc-widget-content fc-past' colspan='8'"+
				" align='center' style='font-size:15px;font-weight:bold'><a href='#' onclick=loadUI('WEB-INF/views/schedule/schedulerWeekGroup.html',"+JSON.stringify(options)+")>"+groupName+"</a></td></tr>";
			}
			
			html+=buildWeekCalendarHTML();
			...
			for (var j = 0; j < xhyList.length; j++) {
				...
				if(pname.length==2){
					pname=pname.substring(0,1)+"&nbsp;&nbsp;&nbsp;&nbsp;"+pname.substring(1);
				}
				html += "<tr class='fc-week'><td align='center' width='120px' class='fc-wed fc-widget-content fc-past'"+
				" style='vertical-align: middle;font-weight: bold;color:gray;font-size: 16px;'><a href='#' onclick=loadUI('WEB-INF/views/schedule/schedulerMonthSingle.html',"+JSON.stringify(options)+")><i class='glyphicon glyphicon-user'></i> "+pname+"</a></td>";
	...
	function buildWeekCalendarHTML(){
		...
		html += "<tr><th class='fc-day-header fc-sun fc-widget-header'>日期</th>";
		...
		for (col=0; col<colCnt; col++) {
			date = cellToDate(0, col);
			var dutyNum=0,restNum=0,dateTmp=formatDate(date, 'yyyy-MM-dd');
			if(null!=Tiger.pbglEventsDateCache[dateTmp]){
				for(var idx=0;idx<Tiger.pbglEventsDateCache[dateTmp].length;idx++){
					//夜班换休，白班换休，周末休息以及夜班的人数为当天不在单位的人数
					if(-1!=(Tiger.pbglEventsDateCache[dateTmp][idx].indexOf("休"))||-1!=(Tiger.pbglEventsDateCache[dateTmp][idx].indexOf("夜班"))){
						restNum++;
					}else{
						dutyNum++;
					}
			...
			html +=
				"<th class='fc-day-header fc-" + dayIDs[date.getDay()] + " " + headerClass + "'>" +
				htmlEscape(formatDate(date, colFormat)) +" ("+restNum+"/"+dutyNum+")"
				"</th>";
		}
	...
	function buildWeekCellHTML(date,pid,personName) {
		...
		var eventsObj=Tiger.xhyEventsObj[pid] || {};
		var events=eventsObj[dayStr]  || "";
		if(events){
			var eventArr=events.split("|");
			for (var bc = 0; bc < eventArr.length; bc++) {
				// 获取该班次的颜色
				var bcColor=Tiger.pbglBcColorObj[eventArr[bc]] || "#fff";
				html+='<div class="schedualitem" style="background-color:'+bcColor+';opacity:0.8;">';
